...
def test_min_valid_conf(
    ...
    result = runner.invoke(cli.app, [get_config_file("min_valid_conf.yaml"), "--debug"])
    assert result.exit_code == os.EX_OK, _format_cli_error(result)
    assert "Image sync took" in caplog_debug.text
...
def test_multi_stage_ids_conf(get_config_file: Callable[[str], str]):
    result = runner.invoke(
        cli.app,
        [get_config_file("multi_stage_ids_conf.yaml"), "--debug"],
    )
    assert result.exit_code != os.EX_OK, _format_cli_error(result)
...
def test_no_such_stage_conf(get_config_file: Callable[[str], str]):
    result = runner.invoke(
        cli.app,
        [get_config_file("no_such_stage_conf.yaml"), "--debug"],
    )
    assert result.exit_code != os.EX_OK, _format_cli_error(result)
...
def test_stages_self_dependency_conf(get_config_file: Callable[[str], str]):
    result = runner.invoke(
        cli.app,
        [get_config_file("stages_self_dependency_conf.yaml"), "--debug"],
    )
    assert result.exit_code != os.EX_OK, _format_cli_error(result)
